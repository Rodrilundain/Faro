/*
* faro_core.js - Nucleo del pipeline FARO reescrito en JavaScript.
*
* Replica EXACTAMENTE la logica de los modulos Python (sanitacion.py,
...
(function (global) {
 ...
 // ---- Constantes del modelo (idénticas a calculo_riesgo.py por defecto) ----
 var DIAS_VENTANA = 28;
  var MIN_REGISTROS = 4;
  var PESOS = {
 ...
 // Si exportar_sitio.py embebio window.FARO_CONFIG (leido de
 // config/riesgo_config.json) en datos_faro.js, lo aplicamos aca para que
 // Python y JS no puedan desincronizarse. Si no existe (por ejemplo en los
 // tests de Node sin ese archivo cargado), se mantienen los valores de arriba.
 function _aplicarConfig() {
   var cfg = (typeof global !== "undefined" && global.FARO_CONFIG) ? global.FARO_CONFIG : null;
   if (!cfg) return;
   if (cfg.dias_ventana) DIAS_VENTANA = cfg.dias_ventana;
   if (cfg.min_registros) MIN_REGISTROS = cfg.min_registros;
   if (cfg.pesos) PESOS = cfg.pesos;
   if (cfg.umbrales) {
     if (cfg.umbrales.rojo !== undefined) UMBRAL_ROJO = cfg.umbrales.rojo;
     if (cfg.umbrales.naranja !== undefined) UMBRAL_NARANJA = cfg.umbrales.naranja;
     if (cfg.umbrales.amarillo !== undefined) UMBRAL_AMARILLO = cfg.umbrales.amarillo;
   }
 }
 ...
 function parseCSV(texto) {
   ...
   for (var r = 1; r < filas.length; r++) {
     if (filas[r].length === 1 && filas[r][0] === "") continue; // linea vacia
   var obj = {};
 ...
 function validarFila(fila) {
   ...
   if (!esNumero(fila.valencia)) return "valencia no es numerica";
   if (!esNumero(fila.activacion)) return "activacion no es numerica";
   ...
   if (!parseFecha(fila.fecha_hora)) return "fecha_hora invalida";
   return null;
 }
 ...
 function senalesDe(f, resumen) {
   ...
   if (f.prop_apagamiento >= 0.4) s.push(Math.round(resumen.prop_apagamiento * 100) + "% de registros de apagamiento (baja energia + animo bajo, cuadrante Triste).");
   ...
   if (f.tendencia >= 0.5) s.push("Tendencia a empeorar en las ultimas semanas.");
   if (f.intensidad_negativa >= 0.6) s.push("Emociones negativas de alta intensidad.");
   if (!s.length) s.push("Sin senales emocionales de alerta relevantes.");
   return s;
 }
 ...
 // ---- PASO 4/enriquecimiento: unir estudiantes + historial + resumenes ----
 function construirResultado(csvTexto, referencia) {
  ...
  // Historial por estudiante (cronologico, ultimos 30).
  var hist = {};
